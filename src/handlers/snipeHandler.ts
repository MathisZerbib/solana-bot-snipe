...
//   adjustStopLoss,
// } from "./tradeHandler";
// import { convertWSolToUSD, getTokenInfo } from "../utils/priceUtils";
...
//     const entryPriceInSOL: number = await getTokenInfo(tokenAddress);
...
//     const currentTokenPriceInUSD: number = await convertWSolToUSD(await getTokenInfo(tokenAddress));


//     console.log("Entry price in USD", entryPriceInUSD);
//     console.log("Entry price in SOL", entryPriceInSOL);

//     const result: MonitorResult = await monitorToken(tokenName, tokenAddress, entryPriceInUSD, CONFIG.amountToSwap);
